            ...
            try {
                ...
                while (condition == false) {
                    ...
                    if (number == 1) {
                        ...
                        String basePath = "/Users/irayatseyko/IdeaProjects/JavaStudyProject/src/main/resources/%s.csv";
                        scan.nextLine();
                        String pathName = String.format(basePath, scan.nextLine());
                        List<Employee> employee1 = readEmployeeDataFromCSV(pathName);
                        Collections.sort(employee1, factoryComparator.createComparator("name"));
                        for (Employee e : employee1) {
                            System.out.println(e);
                        }
                        System.out.println("\nPlease, type a name of the second file from which you want to read employee's data");

                        pathName = String.format(basePath, scan.nextLine());
                        List<Employee> employee2 = readEmployeeDataFromCSV(pathName);
                        Collections.sort(employee2, factoryComparator.createComparator("BirthDate"));
